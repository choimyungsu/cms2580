...
public class JsonUtil {
     ...
     * @auther ddakker 2013. 12. 12.
     */
    public static List<Map<String, Object>> convertorTreeMap(final List<Map<String, Object>> list, String rootId, final String idKey, final String pIdKey, final String titleKey,final String bookId){
        return convertorTreeMap(list, rootId, idKey, pIdKey, titleKey, null, bookId);
    }
    /**
     * 2차원 배열의 부모/자식 관계의 데이터를 트리형식으로 재나열 한다.
     * @param list          2차원 배열
     ...
     * @auther ddakker 2013. 12. 12.
     */
    public static List<Map<String, Object>> convertorTreeMap(List inList, String rootId, final String idKey, final String pIdKey, final String titleKey, final String orderKey ,final String bookId){
        ...
        while ( treeLength[0] != listLength && listLength != loopLength++ ) {
            ...
            for ( int i=0; i<list.size(); i++ ) {
                ...
                if ( rootId.equals((String)item.get(pIdKey)) ) {
                    ...
                    //view.put("label", "'"+item.get(titleKey)+"'"); // 'aaaa' 이런 형태로 반환해줘야 함. title이 아닌 text로 키값도 변경
                    //String urladdlabel = "'<a href=contentDetail.jsp?contentId="+item.get(idKey)+"&bookId="+ bookId + ">"+item.get(titleKey)+"</a>'";
                    String urladdlabel = "'<a href###contentDetail.jsp?contentId###"+item.get(idKey)+"&bookId###"+ bookId + ">"+item.get(titleKey)+"</a>'";
                    view.put("label", urladdlabel); // 'aaaa' 이런 형태로 반환해줘야 함., URL링크 적용
                    ...
                    //view.put("isFolder", "true"); // 필요없으모로 주석
                        ...
                        public void getParentNode(List<Map<String, Object>> children, Map<String, Object> item ) {
                            for ( int i=0; i<children.size(); i++ ) {
                                ...
                                if ( child.get(idKey).equals(item.get(pIdKey)) ) {
                                    ...
                                    String urladdlabel = "'<a href###contentDetail.jsp?contentId###"+item.get(idKey)+"&bookId###"+ bookId + ">"+item.get(titleKey)+"</a>'";
                                    view.put("label", urladdlabel); // 'aaaa' 이런 형태로 반환해줘야 함., URL링크 적용
                                    //view.put("label", item.get(titleKey)); // 링크 없이는 정상 동작 
