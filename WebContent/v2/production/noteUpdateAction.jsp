        ...
        if(session.getAttribute("userID") !=null ){
            ...
            if(userID ==null) {
                ...
                if(content.getTitle() == null || content.getText() == null ) {
                    ...
                    int result = contentDAO.updateContent2(content.getId(), content.getTitle(), content.getText(), content.getContentsImgUrl(), content.getPid(), content.getSeq(),content.gettdate()) ;//(content.getBookId(),content.getTitle(),content.getText(), content.getContentsImgUrl(),content.getPid(),content.gettdate());
                    ...
                    if(result == -1){
                        PrintWriter script = response.getWriter();
                        script.println("<script>");
                        script.println("alert('Note 수정에 실패했습니다.')");
                        script.println("history.back()");
                        script.println("</script>");                    
                    }else{
                        PrintWriter script = response.getWriter();
